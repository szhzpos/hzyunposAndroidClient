...
import static com.wyc.cloudapp.logger.Logger.ASSERT;
import static com.wyc.cloudapp.logger.Logger.DEBUG;
import static com.wyc.cloudapp.logger.Logger.ERROR;
import static com.wyc.cloudapp.logger.Logger.INFO;
...
class LoggerPrinter implements Printer {
  ...
  @Override public void addAdapter(@NonNull LogAdapter adapter) {
    ...
    if (logAdapters.isEmpty()){
      logAdapters.add(checkNotNull(adapter));
      return;
    }
    for (LogAdapter adapter1 : logAdapters) {
        if (adapter1.getClass().getSimpleName().equals(adapter.getClass().getSimpleName())){
          isExist = true;
        }
    }
    if (!isExist)
