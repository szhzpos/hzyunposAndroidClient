        ...
        for (int i = 0,length = mDatas.size();i < length;i++){
            ...
            if (barcode_id == tmp_obj.getIntValue("barcode_id") && gp_id == tmp_obj.getIntValue("gp_id") && sale_type == tmp_obj.getIntValue("sale_type")){
                ...
                if (isBarcodeWeighingGoods){
                    ...
                    current_discount_amt = current_sale_amt - current_sale_amt * discount;
        ...
        if (cur_json != null){
            ...
            switch (type){
                case 1:
                    ...
                    break;
                case 2:
                    dialog = new ChangeNumOrPriceDialog(mContext,Html.fromHtml("折扣率<size value='14'>[1-10],10为不折扣</size> ",null,new FontSizeTagHandler(mContext)),String.format(Locale.CHINA,"%.0f",Utils.getNotKeyAsNumberDefault(cur_json,"discount",1.0) * 10));
                    break;
                    case 0:
    ...
    private void updateSaleGoodsInfo(double value,int type){//type 0 修改数量 1修改价格 2打折 3赠送
        ...
        switch (type){
            ...
            case 2:
                ...
                Logger.d("current_sale_amt :%f / xnum:%f",current_sale_amt,xnum);
                ...
                switch (jsonObject.getIntValue("yh_mode")){
                    ...
                    case 1://会员价
                        ...
                        if (d_discount){
                            ...
                            new_price = Utils.formatDouble(new_price * discount,4);
                        }else {
                        ...
                        break;
                    case 2://会员折扣
                        discount  = Utils.formatDouble(Utils.getNotKeyAsNumberDefault(vip,"discount",1.0) / 10,4);
                        if (d_discount){
                            current_sale_amt = old_sale_amt * discount;
                        }else{
                            clearDisoucntRecord(jsonObject);
    ...
    private void clearDisoucntRecord(final JSONObject goods){
        if (null == goods){
            ...
            for (int i = 0,size = mDatas.size();i < size;i++){
                ...
                if (null != obj)deleteDiscountRecordForGoods(obj,0);
            }
        }else {
            deleteDiscountRecordForGoods(goods,0);
        }
        notifyDataSetChanged();
    }
